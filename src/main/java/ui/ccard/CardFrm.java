...
import framework.command.impl.GenBillCommand;
...
public class CardFrm extends javax.swing.JFrame {
	...
	void loadAccounts() {
		Collection<Account> accounts = CreditDAOImpl.getInstance().getAccounts();
		Object[] rowInfo = new Object[7];

		for (Account account : accounts) {
			rowInfo[0] = account.getCustomer().getName();
	...
	private void addCreditAccount() {
		...
		// clientName,street,city, zip, state,accountType,expdate, ccnumber
		dataMap.setName(clientName);
		dataMap.setStreet(street);
		dataMap.setCity(city);
		dataMap.setZip(zip);
		dataMap.setState(state);
		dataMap.setEmail(email);
		...
		if ("Gold".equals(accountType)) {
			dataMap.setAccounTypeEnum(AccounTypeEnum.GOLD);
		} else if ("Silver".equals(accountType)) {
			dataMap.setAccounTypeEnum(AccounTypeEnum.SILVER);
		} else if ("Bronze".equals(accountType)) {
			dataMap.setAccounTypeEnum(AccounTypeEnum.BRONZE);
		}

		dataMap.setCardExpirationDate(expdate);
		dataMap.setCardNumber(ccnumber);
	...
	void JButtonGenerateBill_actionPerformed(java.awt.event.ActionEvent event) {
		...
		GenBillCommand genBillCommand = new GenBillCommand(accountService);

		commandInvoker.execute(genBillCommand);

		JDialogGenBill billFrm = new JDialogGenBill(null, genBillCommand.getBillString());
