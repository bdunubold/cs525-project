...
public class BankFrm extends JFrame {
	...
	private void addPersonalAccount() {
		...
		// PersonalAccountCreationCommand pcCommand = new
		// PersonalAccountCreationCommand(accountService,account);
	...
	private void addCompanyAccount() {
		// TODO parameters checking
		Address address = new Address(street, city, state, zip);
		// Company company = new Company(clientName, email,
		// Integer.parseInt(noOfEmployee), address);
		//
		// Account account = new Account(accountnr);
		// account.setCustomer(company);
		// account.setAccountType("Ch".equals(accountType) ? new
		// CheckingAccount() : new SavingAccount());
		// account.setInterestStrategy("Ch".equals(accountType) ? new
		// CheckingInterest() : new SavingInterest());
		//
		// CompanyAccountCreationCommand pcCommand = new
		// CompanyAccountCreationCommand(accountService,account);
		//
		// commandInvoker.setCommand(pcCommand);
		// commandInvoker.execute();
	}
